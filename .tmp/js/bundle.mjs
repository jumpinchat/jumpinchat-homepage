(function () {
	...
	/* global document */
	const profileButton = jquery('#profile-drop-trigger');
	const profileDropdown = jquery('#profile-drop-container');
	const newButton = jquery('#new');
	...
	const removeModButton = jquery('.settings__RoomModRemove');

	const removeMod = (username, element) => {
	  jquery.ajax({
	    method: 'delete',
	    url: '/api/settings/moderator',
	    data: {
	      username
	    }
	  }).done(() => {
	...
	const userIgnoreRemove = jquery('.settings__UserIgnoreRemove');

	const removeIgnore = (id, username, element) => {
	  jquery.ajax({
	    method: 'delete',
	    url: '/api/settings/ignore',
	    data: {
	      id,
	      username
	    }
	...
	const resendButton = jquery('#send-verification-email');
	const isVerifiedString = jquery('#is-verified-string');
	...
	const usernameInput = jquery('#register-form [name="username"]');
	const usernameText = jquery('#register-form .register__UsernameText');
	const registerSubmit = jquery('#register-form [type="submit"]');

	const checkUsername = (username, cb) => {
	  jquery.ajax({
	    method: 'get',
	    url: "/api/user/checkusername/".concat(username)
	  }).fail(res => cb(res.responseJSON)).done(cb);
	};
	...
	usernameInput.on('change', function onUsernameInputChange() {
	  ...
	  checkUsername(elem.val(), data => {
	    if (data && data.error) {
	      ...
	      return usernameText.removeClass('text--green').addClass('text--red').html(data.message);
	    }

	    registerSubmit.prop('disabled', false);
	    return usernameText.removeClass('text--red').addClass('text--green').html('Username available!');
	  });
	});

	let modal;
	jquery('.modal-trigger').on('click', function openModal(e) {
	  const target = jquery(this).attr('data-target');
	  modal = jquery("#".concat(target));
	  modal.addClass('open');
	});
	jquery('.modal-btn-close').on('click', e => {
	...
	jquery(document).ready(() => {
	  const timeElements = jquery('time.convertDate');
	  timeElements.each(function () {
	    const dateText = jquery(this).text();
	    const date = new Date(dateText);
	    jquery(this).html(hooks(date).calendar());
	    jquery(this).attr('datetime', dateText);
	  });
	});

	jquery(document).ready(() => {
	  const button = jquery('button.disableOnClick');
	...
	class ImageUpload {
	  constructor() {
	    let elemSelector = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '.imageUpload__Form';
	  ...
	  createImageUpload() {
	    if (this.isAdvancedUpload()) {
	      const $elem = jquery(this.targetElement);
	      const $form = $elem;
	      const $container = jquery("".concat(this.elemSelector, " .imageUpload__Container"));
	      const $input = jquery("".concat(this.elemSelector, " .imageUpload__File"));
	      const $submit = jquery("".concat(this.elemSelector, " .imageUpload__UploadButton"));
	      const $loading = jquery("".concat(this.elemSelector, " .imageUpload__Loading"));
	      const $statusSuccess = jquery("".concat(this.elemSelector, " .imageUpload__Status--success"));
	      const $statusError = jquery("".concat(this.elemSelector, " .imageUpload__Status--error"));
	      ...
	      $elem.on('drag dragstart dragend dragover dragenter dragleave drop', e => {
	        e.preventDefault();
	        e.stopPropagation();
	      ...
	      this.reader.addEventListener('load', () => {
	        ...
	        $elem.find('.imageUpload__Label').addClass('imageUpload__Label--hasImage');
